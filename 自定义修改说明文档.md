...
在实施了“构建时内存缓存”策略（第10节）后，虽然成功解决了 `429` API 请求过多的问题，并大幅提升了构建速度，但 Vercel 平台的部署开始持续失败，并报告以下错误：
`Error: A Serverless Function has exceeded the unzipped maximum size of 250 MB.`

经分析，这是因为 Vercel 在打包部署产物时，将我们存储在 `global` 变量中的巨大缓存对象（包含了整个博客的所有数据）也一并打包进了每一个 Serverless Function 中，导致函数体积严重超标。

#### 11.2 解决方案演进

为了解决此问题，我们必须将“构建时”的缓存与“运行时”的产物彻底分离。

**第一版尝试：项目内文件缓存 (失败)**
- **思路**: 将内存缓存改为文件缓存，写入到项目内的 `.next/cache/site-data.json`。
- **结果**: 失败。Vercel 的打包器依然会将 `.next/cache` 目录下的内容识别为依赖项并打包，问题依旧。

**第二版尝试：操作系统临时目录缓存 (最终成功)**
- **思路**: 将所有在构建过程中产生的临时缓存文件，全部存放到一个 Vercel 打包器绝对不会扫描的外部路径——操作系统的临时目录（如 `/tmp`）。
- **关键疏忽与修正**: 初次尝试时，只将主要的 `site-data.json` 缓存移到了临时目录，但遗漏了为 sitemap 生成的 `lastmod-map.json` 文件，导致问题依旧。最终，我们将 **所有** 动态生成的缓存文件都移到了临时目录，才彻底解决了问题。

#### 11.3 修改的文件

**1. lib/db/getSiteData.js**

- 引入 Node.js 的 `os` 模块。
- 将 `site-data.json` 和 `lastmod-map.json` 的读写路径全部指向 `os.tmpdir()`。
...
- 引入 Node.js 的 `os` 模块。
- 修改 `getLastmodMap` 函数，使其从 `os.tmpdir()` 读取 `lastmod-map.json` 文件。
...
- **Vercel 部署成功**: `250 MB` 体积超限的错误彻底消失，Vercel 平台恢复正常部署。
- **双平台兼容**: 新的缓存策略同时兼容 Vercel 的 Serverless 部署模型和基于 `npm run export` 的静态导出部署模型（用于阿里云）。
- **保留性能优势**: 依然享受着构建时缓存带来的 API 请求稳定和构建速度快的优点。
...
- **主要功能**: 
  ...
  - Vercel 部署优化（解决 Serverless Function 体积超限）（2025年10月10日新增）
