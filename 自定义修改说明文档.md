...
在构建过程中，我们会生成一些体积较大的缓存文件（如 `lastmod-map.json`）来提升性能和实现特定功能。然而，Vercel 在打包部署产物时，会将这些缓存文件也一并打包进 Serverless Function 中，导致函数体积轻易超过 `250 MB` 的上限，从而引发部署失败。

#### 11.2 解决方案演进与最终方案

**第一版尝试：操作系统临时目录缓存 (`/tmp`) (部分失败)**
- **思路**: 将缓存文件写入 Vercel 不会打包的 `/tmp` 目录。
- **结果**: 此方案解决了部分问题，但导致了 Sitemap 生成失败。因为在 Vercel 的构建环境中，`next build` 和后续的 `node scripts/merge-sitemap.js` 是在**相互隔离**的步骤中运行的，后一个步骤无法访问前一个步骤在 `/tmp` 中创建的文件。

**第二版尝试：项目内缓存 + `.vercelignore` (最终成功)**
- **思路**: 必须将缓存文件写入项目内部的持久化路径，以确保在构建步骤之间可以共享。同时，必须有一种方法告诉 Vercel 在打包时忽略这些缓存文件。
- **最终方案**:
    1.  **统一缓存路径**: 将所有构建时缓存（特别是 `lastmod-map.json`）统一写入项目内的 `.next/cache/` 目录。这确保了构建步骤之间的数据可见性。
    2.  **创建 `.vercelignore`**: 在项目根目录创建 `.vercelignore` 文件。
    3.  **配置忽略规则**: 在 `.vercelignore` 文件中添加 `.next/cache`，明确指示 Vercel 在打包 Serverless Function 时忽略此目录及其所有内容。
...
在项目根目录创建此文件，内容如下：
```
# Ignore the entire Next.js cache directory from Vercel deployments.
# This prevents the large `lastmod-map.json` and other cache files
# from being bundled into the serverless function, solving the size limit issue.
.next/cache
```

**2. 确保缓存路径正确**

- **lib/db/getSiteData.js**: 确保 `lastmod-map.json` 的写入路径指向 `.next/cache/`。
- **scripts/merge-sitemap.js**: 确保 `lastmod-map.json` 的读取路径指向 `.next/cache/`。

#### 11.4 实现效果

- **Vercel 部署成功**: `250 MB` 体积超限的错误被彻底解决。
- **构建流程健壮**: Sitemap 生成流程所需的文件持久化得到保证，不再因构建环境隔离而失败。
- **两全其美**: 完美实现了在构建步骤间共享文件，同时避免了部署产物体积超限的问题，是解决此类问题的最终方案。
...
- **主要功能**: 
  ...
  - Vercel 部署优化（通过`.vercelignore`解决体积超限）（2025年10月11日更新）
