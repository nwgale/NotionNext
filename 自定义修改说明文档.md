...
- **问题**: 网站存在一个通过 Nginx 反向代理的独立静态目录 `/app/`，该目录下的页面无法被 NotionNext 自动生成的 `sitemap.xml` 所收录。
- **解决方案**: 采用“以Notion为中心”的思路，在 Notion 配置中心数据库中新增一个 `key` 为 `APP_LIST_FOR_SITEMAP` 的配置项。
- **维护方式**:
    - 在该配置项的 `value` 中，以**换行（回车）**分隔的方式，填入需要加入 Sitemap 的所有外部页面路径（例如 `/app/yuliu/`）。
    - 每次更新 Notion 配置并重新部署后，Sitemap 将会自动包含这些新路径。
- **详细文档**: 具体的方案演进和代码实现细节，请参阅笔记 `如何为NotionNext网站的sitemap添加额外目录？.md`。
...
- **主要功能**: 
  - 通过在 Notion 配置中心添加 `APP_LIST_FOR_SITEMAP` 字段，实现了将外部独立目录动态添加到 `sitemap.xml` 的功能。（2025年9月16日新增）
  - 教育版页面、页脚链接配置、样式优化
