...
// import { useGlobal } from './global' // useGlobal is a client-side hook, cannot be used in CJS/Node.js script
import { deepClone, isUrlLikePath } from './utils/index.js'
...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  // In server-side script like next-sitemap, we can't use client-side hooks like useGlobal.
  // The logic will fall back to reading from extendConfig and blog.config.js
  ...
  if (!isNodeEnv) {
    try {
      ...
      global = require('./global').useGlobal()
      if (global) {
        siteInfo = global.siteInfo
        val = global.NOTION_CONFIG?.[key] || global.THEME_CONFIG?.[key]
      }
    } catch (e) {
      // console.warn('Cannot use useGlobal in this context', e)
    }
  }
