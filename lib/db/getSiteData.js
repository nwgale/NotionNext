...
function handleDataBeforeReturn(db) {
  ...
  try {
    ...
    // Create a map for the current batch of pages
    const newLastmodMap = {}
    db.allPages.forEach(p => {
      // Use p.href as the key, which is the final, canonical path for the page.
      if (p.href && p.href !== '/') {
        // Prioritize lastEditedDate, fall back to publishDate
    ...
    // Write the map to the file, overwriting any existing one.
    // Merging is not necessary here because getGlobalData provides all pages.
    fs.writeFileSync(filePath, JSON.stringify(newLastmodMap))
    console.log(`[Build] lastmod-map.json generated with ${Object.keys(newLastmodMap).length} entries.`)
  } catch (e) {
