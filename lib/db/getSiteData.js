    ...
    db.allPages.forEach(p => {
      if (p.href && p.href !== '/') {
        ...
        if (lastmod) {
          ...
          // Extensive debugging to be absolutely sure
          if (p.type === 'Post' && p.status === 'Published') {
            if (p.lastEditedDate) {
              console.log(`[Sitemap Writer] OK: Using lastEditedDate ${new Date(p.lastEditedDate).toISOString()} for ${p.href}`)
            } else {
              console.log(`[Sitemap Writer] FALLBACK: lastEditedDate is MISSING for ${p.href}. Using publishDate ${p.publishDate}`)
            }
          }
        }
      }
