...
const isFallbackSiteData = data =>
  data?.allPages?.some(
    page => page?.id === 'oops-error-page' || page?.slug === 'oops'
  )

const isEmptySiteData = data =>
  Array.isArray(data?.allPages) && data.allPages.length === 0

const isUsableSiteData = data =>
  !!data && !isEmptySiteData(data) && !isFallbackSiteData(data)

const normalizeNotionPageId = rawPageId => {
  const pageId = typeof rawPageId === 'string' ? rawPageId.trim() : rawPageId
  if (!pageId) {
    return rawPageId
  }
  // Already UUID format, do not normalize again.
...
export async function getGlobalData({
  ...
  // Use file cache in OS temp directory to avoid Vercel's serverless function size limit
  const crypto = require('crypto')
  const fs = require('fs')
  const path = require('path')
  const os = require('os')
  const cacheFileHash = crypto
    .createHash('md5')
    .update(String(pageId || BLOG.NOTION_PAGE_ID))
    .digest('hex')
    .slice(0, 12)
  const cacheFilePath = path.join(
    os.tmpdir(),
    `notion-next-site-data-${cacheFileHash}.json`
  )
  // If the cache file exists, read from it
  if (fs.existsSync(cacheFilePath)) {
    try {
      const cachedData = JSON.parse(fs.readFileSync(cacheFilePath, 'utf-8'))
      if (isUsableSiteData(cachedData)) {
        console.log('[Build Cache] Hit: Returning data from file cache.')
        return handleDataBeforeReturn(deepClone(cachedData))
      }
      console.warn(
        '[Build Cache] Invalid fallback cache detected, ignore and refetch.'
      )
      fs.unlinkSync(cacheFilePath)
    } catch (error) {
      console.warn('[Build Cache] Cache file is broken, ignore and refetch.')
      fs.unlinkSync(cacheFilePath)
    }
  }
  ...
  try {
    for (let index = 0; index < siteIds.length; index++) {
      ...
      const id = extractLangId(siteId)?.trim()
      const prefix = extractLangPrefix(siteId)
      // The first id site default language
      if (index === 0 || locale === prefix) {
        let candidateData = null
        const retryTimes = 3
        for (let attempt = 1; attempt <= retryTimes; attempt++) {
          candidateData = await getSiteDataByPageId({
            pageId: id,
            from
          })
          if (Array.isArray(candidateData?.allPages) && candidateData.allPages.length === 0) {
            console.warn('[Build Data] Candidate site data is empty', {
              from,
          ...
          if (isUsableSiteData(candidateData)) {
            break
          }
          console.warn(
            `[Build Data] Attempt ${attempt}/${retryTimes} got fallback data for pageId=${id}, retrying...`
          )
  ...
  if (Array.isArray(data?.allPages) && data.allPages.length === 0) {
    console.error('[Build Data] Final site data allPages is empty', {
      from,
      locale: locale || 'default',
      pageId
    })
  ...
  // In CI/export pipeline, fallback/empty data means build result is invalid.
  if (
    (isFallbackSiteData(data) || isEmptySiteData(data)) &&
    (process.env.EXPORT || process.env.CI)
  ) {
    throw new Error(
      '[Build Fatal] Notion data unavailable, fallback or empty data detected.'
    )
  }

  // Write data to file cache for subsequent build processes to use
  const cacheDir = path.dirname(cacheFilePath)
  if (!fs.existsSync(cacheDir)) {
    fs.mkdirSync(cacheDir, { recursive: true })
  }
  if (isUsableSiteData(data)) {
    fs.writeFileSync(cacheFilePath, JSON.stringify(data))
    console.log('[Build Cache] Miss: Fetched data and cached to file.')
  } else {
    console.warn(
      '[Build Cache] Skip caching fallback data to avoid poisoning later pages.'
    )
  }
...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  // 先读缓存；如果是兜底数据，立即清理，避免污染后续流程
  const dataFromCache = await getDataFromCache(cacheKey)
  if (dataFromCache) {
    if (isFallbackSiteData(dataFromCache)) {
      console.warn(
        `[Build Cache] Found fallback site_data cache, deleting key=${cacheKey}`
      )
  ...
  const siteData = await convertNotionToSiteData(pageId, from, deepClone(pageRecordMap))
  if (Array.isArray(siteData?.allPages) && siteData.allPages.length === 0) {
    console.error('[Build Data] getSiteDataByPageId got empty allPages', {
      from,
      pageId
    })
  }

  // 不缓存兜底数据，避免一次失败污染整次构建
  if (isUsableSiteData(siteData)) {
    await setDataToCache(cacheKey, siteData)
  } else {
    console.warn(
      `[Build Cache] Skip caching fallback site data for key=${cacheKey}`
    )
  }
...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  if (pageIds?.length === 0) {
    console.error('[Build Data] pageIds resolved to empty', {
      from,
      pageId,
      collectionId,
  ...
  const unresolvedPageIds = pageIds.filter(id => !block[id]?.value)
  if (unresolvedPageIds.length > 0) {
    console.error('[Build Data] Missing page blocks after recovery', {
      from,
      pageId,
      missingCount: unresolvedPageIds.length,
      sampleIds: unresolvedPageIds.slice(0, 10)
    })

    if (process.env.EXPORT || process.env.CI) {
      throw new Error(
        `[Build Fatal] Missing ${unresolvedPageIds.length} Notion page blocks after recovery`
      )
    }
  }
...
function handleDataBeforeReturn(db) {
  // Generate lastmod-map.json for sitemap, must be here to run on every build
  try {
    const fs = require('fs')
    const path = require('path')
    // Use a persistent cache directory within the project
    const cacheDir = path.resolve('.next', 'cache')
    // Ensure the cache directory exists
    fs.mkdirSync(cacheDir, { recursive: true })
    const filePath = path.join(cacheDir, 'lastmod-map.json')

    // 1. Read existing map if it exists
    let existingLastmodMap = {}
    if (fs.existsSync(filePath)) {
      try {
        existingLastmodMap = JSON.parse(fs.readFileSync(filePath, 'utf-8'))
      } catch (e) {
        // If the file is corrupted or empty, start fresh
        console.warn('[Build] Failed to parse existing lastmod-map.json, will create a new one.', e)
        existingLastmodMap = {}
      }
    }

    // 2. Create a map for the current batch of pages
    const newLastmodMap = {}
    db.allPages.forEach(p => {
      if (p.href && p.href !== '/') {
        const lastmod = p.lastEditedDate || p.publishDate
        if (lastmod) {
          newLastmodMap[p.href] = lastmod
        }
      }
    })

    // 3. Merge new map into existing map
    const mergedMap = { ...existingLastmodMap, ...newLastmodMap }

    // 4. Write the merged map back to the file
    fs.writeFileSync(filePath, JSON.stringify(mergedMap))
    console.log(`[Build] lastmod-map.json updated. Added/updated ${Object.keys(newLastmodMap).length} entries. Total entries: ${Object.keys(mergedMap).length}.`)
  } catch (e) {
    console.warn('[Build] Failed to generate lastmod-map.json for sitemap.', e)
  }
