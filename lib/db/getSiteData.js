...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  // Generate lastmod-map.json for sitemap
  try {
    const fs = require('fs')
    const path = require('path')
    const os = require('os')
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        console.warn('[Build] Failed to parse existing lastmod-map.json, will overwrite.', e)
        existingLastmodMap = {}
      }
    }
    ...
    const newLastmodMap = {}
    allPages.forEach(p => {
      // Use p.href as the key, which is the final, canonical path for the page.
      // Ignore the root path '/' as it's handled by next-sitemap itself.
      if (p.href && p.href !== '/') {
        const lastmod = p.lastEditedDate
        if (lastmod) {
          // Store the raw timestamp number for maximum flexibility and to avoid timezone issues.
          newLastmodMap[p.href] = lastmod
        } else {
          // Ultimate debugging: Log pages that are missing a lastmod value.
          console.log(`[Sitemap Debug] Page "${p.title}" (${p.href}) is missing a lastmod value. Falling back to publishDate. lastEditedDate: ${p.lastEditedDate}, publishDate: ${p.publishDate}`)
          newLastmodMap[p.href] = p.publishDate
        }
      }
    })
    ...
    // The temporary directory is guaranteed to exist, so no need to create it.
    fs.writeFileSync(filePath, JSON.stringify(mergedMap))
    console.log(`[Build] lastmod-map.json updated with ${Object.keys(newLastmodMap).length} new entries. Total: ${Object.keys(mergedMap).length}`)
  } catch (e) {
    console.warn('[Build] Failed to generate or update lastmod-map.json for sitemap.', e)
  }
