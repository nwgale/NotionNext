...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  try {
    ...
    allPages.forEach(p => {
      if (p.slug) {
        // Use lastEditedDate which is more accurate, fallback to publishDate
        const lastmod = p.lastEditedDate || p.publishDate
        // The path should be relative to the domain root, e.g. /article/my-post
        const pagePath = '/' + p.slug
        if (lastmod) {
          lastmodMap[pagePath] = new Date(lastmod).toISOString()
        }
      }
    })
    ...
    fs.writeFileSync(filePath, JSON.stringify(lastmodMap))
    console.log('[Build] lastmod-map.json generated for sitemap.')
  } catch (e) {
