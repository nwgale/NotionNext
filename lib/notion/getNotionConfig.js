...
import { fetchInBatches, getPage } from './getPostBlocks'
...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  // getAllPageIds 可能返回了不在当前 blockMap 中的行，需要补拉取
  const missingPageIds = pageIds.filter(id => !block[id]?.value)
  if (missingPageIds.length > 0) {
    try {
      const fetchedBlocks = await fetchInBatches(missingPageIds)
      Object.assign(block, fetchedBlocks)
      console.warn('[Notion配置] 已补拉取配置行 block', {
        missingCount: missingPageIds.length
      })
    } catch (error) {
      console.error('[Notion配置] 补拉取配置行失败', error)
    }
  }
...
function isConfigEnabled(value) {
  ...
  return ['yes', 'true', '1', 'on', 'y', '是', '启用'].includes(normalized)
}
