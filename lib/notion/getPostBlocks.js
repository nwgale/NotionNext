...
const DEFAULT_PAGE_RETRY_ATTEMPTS =
  process.env.EXPORT || process.env.CI ? 6 : 3
const MAX_NOTION_REQUEST_CONCURRENCY = Number(
  process.env.NOTION_REQUEST_CONCURRENCY ||
    (process.env.EXPORT || process.env.CI ? 2 : 6)
)
let activeNotionRequests = 0
const notionRequestQueue = []

async function withNotionRequestSlot(task) {
  while (activeNotionRequests >= MAX_NOTION_REQUEST_CONCURRENCY) {
    await new Promise(resolve => notionRequestQueue.push(resolve))
  }
  activeNotionRequests++
  try {
    return await task()
  } finally {
    activeNotionRequests = Math.max(0, activeNotionRequests - 1)
    const next = notionRequestQueue.shift()
    if (next) {
      next()
    }
  }
...
function getRetryDelayMs(error, retryAttemptsLeft, initialRetryAttempts) {
  const retryAfterHeader = error?.response?.headers?.get?.('retry-after')
  const retryAfterSeconds = Number.parseInt(retryAfterHeader || '', 10)
  const usedAttempts =
    (initialRetryAttempts || DEFAULT_PAGE_RETRY_ATTEMPTS) - retryAttemptsLeft + 1
  const backoffMs = Math.min(1000 * Math.max(1, usedAttempts), 15000)

  if (Number.isFinite(retryAfterSeconds) && retryAfterSeconds > 0) {
    return Math.max(backoffMs, Math.min(retryAfterSeconds * 1000, 60000))
  }
...
export async function getPageWithRetry(
  id,
  from,
  retryAttempts = DEFAULT_PAGE_RETRY_ATTEMPTS,
  initialRetryAttempts = retryAttempts
) {
  if (retryAttempts && retryAttempts > 0) {
    console.log(
      ...
      `from:${from}`,
      `id:${id}`,
      retryAttempts < initialRetryAttempts ? `剩余重试次数:${retryAttempts}` : ''
    )
    try {
      const start = new Date().getTime()
      const pageData = await withNotionRequestSlot(() => notionAPI.getPage(id))
      const end = new Date().getTime()
      console.log('[API<<--响应]', `耗时:${end - start}ms - from:${from}`)
      return normalizeRecordMap(pageData)
    } catch (e) {
      console.warn('[API<<--异常]:', e)
      const waitMs = getRetryDelayMs(e, retryAttempts, initialRetryAttempts)
      await delay(waitMs)
  ...
  for (let i = 0; i < normalizedIds.length; i += effectiveBatchSize) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        const start = new Date().getTime()
        pageChunk = await withNotionRequestSlot(() => notionAPI.getBlocks(batch))
