...
export default async function getPageProperties(
  ...
  // For sitemap, we need a reliable timestamp.
  // Prioritize `last_edited_time` > `created_time`. Both are full timestamps.
  const lastEditedTime = value?.last_edited_time
  const createdTime = value?.created_time
  properties.lastEditedDate = new Date(lastEditedTime || createdTime).getTime()
  properties.lastEditedDay = formatDate(
    new Date(properties.lastEditedDate),
    BLOG.LANG
  )

  // `publishDate` is for display purposes. It can be overridden by the user with a "Date" property in Notion.
  // If not set, it falls back to the created time.
  properties.publishDate = new Date(
    properties?.date?.start_date || createdTime
  ).getTime()
