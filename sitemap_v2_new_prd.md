...
2.  重构 `scripts` 命令，将 sitemap 生成逻辑打包，并加入到构建流程中。
...
1.  在 `package.json` 的根级别（与 `name`, `version` 同级）添加以下内容：
    ...
    ```
2.  找到 `scripts` 对象，将其**完整替换**为以下内容，以引入新的 `sitemap` 命令并更新 `post-build` 和 `export`：
    ```json
    "scripts": {
      "dev": "next dev",
      "build": "next build",
      "start": "next start",
      "sitemap": "node scripts/generate-sitemap-paths.js && next-sitemap --config next-sitemap.config.js",
      "post-build": "npm run sitemap",
      "export": "cross-env EXPORT=true next build && npm run sitemap",
      "bundle-report": "cross-env ANALYZE=true next build",
      "version": "echo $npm_package_version"
    },
    ```
    **优化说明**:
    *   **修正遗漏**: 新的 `sitemap` 命令确保了 `--config next-sitemap.config.js` 参数不会丢失。
    *   **避免重复**: sitemap 的生成逻辑只在一个地方定义（`sitemap` 命令），`post-build` 和 `export` 都调用它，维护更简单。
    *   **逻辑清晰**: 无论是 `build` 还是 `export`，都会以同样正确的方式生成 sitemap。
